...
*Setup instructions for a gitian build of xsgt using a Debian VM or physical system.*

Gitian is the deterministic build process that is used to build the xsgt
...
Clone the git repositories for xsgt and gitian and then checkout the xsgt version that you want to build.

```bash
git clone https://github.com/devrandom/gitian-builder.git
git clone https://github.com/xsgt-crypto/xsgt.git
cd xsgt
...
Follow the instructions in [doc/release-process.md](release-process.md) in the xsgt repository
...
To build xsgt (for Linux, OSX and Windows) just follow the steps under 'perform
gitian builds' in [doc/release-process.md](release-process.md) in the xsgt repository.
...
```bash
    Initialized empty Git repository in /home/debian/gitian-builder/inputs/xsgt/.git/
    remote: Reusing existing pack: 35606, done.
    remote: Total 35606 (delta 0), reused 0 (delta 0)
    Receiving objects: 100% (35606/35606), 26.52 MiB | 4.28 MiB/s, done.
    Resolving deltas: 100% (25724/25724), done.
    From https://github.com/xsgt-crypto/xsgt
...
For example:
```bash
URL=https://github.com/crowning-/xsgt.git
COMMIT=b616fb8ef0d49a919b72b0388b091aaec5849b96
./bin/gbuild --commit xsgt=${COMMIT} --url xsgt=${URL} ../xsgt/contrib/gitian-descriptors/gitian-linux.yml
./bin/gbuild --commit xsgt=${COMMIT} --url xsgt=${URL} ../xsgt/contrib/gitian-descriptors/gitian-win.yml
./bin/gbuild --commit xsgt=${COMMIT} --url xsgt=${URL} ../xsgt/contrib/gitian-descriptors/gitian-osx.yml
```
...
```bash
    gpg --detach-sign ${VERSION}-linux/${SIGNER}/xsgt-build.assert
    gpg --detach-sign ${VERSION}-win/${SIGNER}/xsgt-build.assert
    gpg --detach-sign ${VERSION}-osx/${SIGNER}/xsgt-build.assert
```
...
[xsgt/gitian.sigs](https://github.com/xsgt-crypto/gitian.sigs/) repository, or if that's not possible to create a pull
request.
