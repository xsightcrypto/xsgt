...
Value getgenerate(const Array& params, bool fHelp)
{
    if (fHelp || params.size() != 0)
        throw runtime_error(
            ...
            "It is set with the command line argument -gen (or xsgt.conf setting gen)\n"
...
Value getblocktemplate(const Array& params, bool fHelp)
{
    ...
    if (vNodes.empty())
        throw JSONRPCError(RPC_CLIENT_NOT_CONNECTED, "xsgt is not connected!");

    if (IsInitialBlockDownload())
        throw JSONRPCError(RPC_CLIENT_IN_INITIAL_DOWNLOAD, "xsgt is downloading blocks...");
...
// Release the wallet and main lock while waiting
    ...
    BOOST_FOREACH (CTransaction& tx, pblock->vtx) {
        ...
        entry.push_back(Pair("sigops", pblocktemplate->vTxsgtOps[index_in_template]));
