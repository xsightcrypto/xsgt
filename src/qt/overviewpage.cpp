...
class TxViewDelegate : public QAbstractItemDelegate
{
    Q_OBJECT
public:
    TxViewDelegate() : QAbstractItemDelegate(), unit(BitcoinUnits::xsgt)
    {
    }
...
void OverviewPage::setWalletModel(WalletModel* model)
{
    ...
    // update the display unit, to not use the default ("xsgt")
    updateDisplayUnit();
}
...
void OverviewPage::updateObfuscationProgress()
{
    ...
    QString strAnonymizexsgtAmount = BitcoinUnits::formatHtmlWithUnit(nDisplayUnit, nAnonymizexsgtAmount * COIN, false, BitcoinUnits::separatorAlways);
    ...
    if (currentBalance == 0) {
        ...
        strAnonymizexsgtAmount = strAnonymizexsgtAmount.remove(strAnonymizexsgtAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizexsgtAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    ...
    if (nMaxToAnonymize > nAnonymizexsgtAmount * COIN) nMaxToAnonymize = nAnonymizexsgtAmount * COIN;

    if (nMaxToAnonymize == 0) return;

    if (nMaxToAnonymize >= nAnonymizexsgtAmount * COIN) {
        ui->labelAmountRounds->setToolTip(tr("Found enough compatible inputs to anonymize %1")
                                              .arg(strAnonymizexsgtAmount));
        strAnonymizexsgtAmount = strAnonymizexsgtAmount.remove(strAnonymizexsgtAmount.indexOf("."), BitcoinUnits::decimals(nDisplayUnit) + 1);
        strAmountAndRounds = strAnonymizexsgtAmount + " / " + tr("%n Rounds", "", nObfuscationRounds);
    } else {
